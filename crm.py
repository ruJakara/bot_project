...
from config import ALFA_API_KEY, ALFA_BASE_URL, ALFA_EMAIL, get_settings
...
def api_request(method: str, path: str, json: Optional[Dict[str, Any]] = None) -> Any:
    ...
    if response.status_code == 401 or "AccessDenied" in response.text:
    ...
    try:
        ...
        logger.error("AlfaCRM request failed: %s", exc)
        raise
...
def create_lead(payload: Dict[str, Any]) -> Union[int, Dict[str, Any]]:
    lead_payload = {
        "name": payload.get("name"),
        "phone": payload.get("phone"),
        "comment": payload.get("comment"),
        "source": payload.get("source"),
        "custom_fields": {"tg_user_id": payload.get("tg_user_id")},
    }

    try:
        response = api_request("POST", "v2api/leads", json=lead_payload)
    except Exception as exc:
        logger.error("Failed to create AlfaCRM lead: %s", exc)
        return {"error": str(exc)}

    lead_id = None
    if isinstance(response, dict):
        lead_id = response.get("id") or response.get("data", {}).get("id")

    if lead_id is None:
        return {
            "error": "AlfaCRM lead ID not found in response",
            "response": response,
        }
